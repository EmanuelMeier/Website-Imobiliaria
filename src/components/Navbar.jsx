      ...
      <div className={`md:hidden ${showMobileMenu ? 'fixed w-full' : 'h-0 w-0'}
       right-0 top-0 bottom-0 overflow-hidden bg-white transition-all`}>
        ...
        <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-lg font-medium'>
          ...
          <a  onClick={()=> setShowMobileMenu(false)} href="#Projetos" className='px-4 py-2 rounded-full inline-block'>Projetos</a>
          <a  onClick={()=> setShowMobileMenu(false)} href="#Depoimentos" className='px-4 py-2 rounded-full inline-block'>Depoimentos</a>
        </ul>
      </div>
    </div>
  )
